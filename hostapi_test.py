...
def get_title(res,soup,wd,query):
    accuracy=0
    #Get info
    #Print all titles
    ...
    if "Sorry! Something went wrong!" in wd.get_title():
        ...
        for i in range(len(mywords)):
            temp_clean=""
            current_word=mywords[i].lower()
            for j in range(len(current_word)):
                if (current_word[j]>="a" and current_word[j]<="z") or (current_word[j]>="0" and current_word[j]<="9"):
                    temp_clean=temp_clean+current_word[j]
            totalwords=totalwords+" "+temp_clean
        ...
        if wd.get_claim()!="none":
            for i in range(len(querysplit)):
                if querysplit[i] in wd.get_claim():
                    accuracy+=1
        if wd.get_claim()=="none" or accuracy==0:
...
def main():
    ...
    class Factual(Resource):
        def get(self,question):
            return start(question)
    
    api.add_resource(Factual,"/factual/<string:question>")
